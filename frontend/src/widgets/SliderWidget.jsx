...
const STREAM_INTERVAL_MS = 60; // throttle: send at most ~16 cmd/s while dragging
...
export default function SliderWidget({ title, value, widget, onCommand, settings }) {
  ...
  function handleChange(e) {
    ...
    // Throttle: send immediately if interval has passed, else schedule
    const now = Date.now();
  ...
  function handleRelease(e) {
    ...
    setDragging(false);
    sendCommand(v); // always send final value on release
  }

  const pct = ((val - min) / (max - min)) * 100;

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8px 18px', gap: 8 }}>
      {/* Title + live value */}
      ...
      {/* Fill indicator bar */}
      <div style={{ width: '100%', height: 3, background: '#1e293b', borderRadius: 99, overflow: 'hidden' }}>
        <div style={{
