...
export default function LEDWidget({ title, value, settings }) {
  ...
  if (ledMode === 'pwm') {
    ...
    brightness = pwmMax > pwmMin ? Math.min(1, Math.max(0, (v - pwmMin) / (pwmMax - pwmMin))) : 0;
  } else {
      ...
      <div style={{ position: 'relative', width: 52, height: 52 }}>
        ...
        <div style={{
          ...
          background: on
            ...
            : `radial-gradient(circle at 35% 35%, var(--w-off-thumb) 0%, var(--w-off) 60%)`,
          boxShadow: on
        ...
        <div style={{
          position: 'absolute', top: 8, left: 10, width: 14, height: 10,
          borderRadius: '50%', background: 'rgba(255,255,255,0.35)', transform: 'rotate(-20deg)',
          opacity: brightness * 0.9, transition: 'opacity 0.25s',
        }} />
      </div>

      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1px', color: on ? color : 'var(--w-text-muted)', transition: 'color 0.25s' }}>
        {ledMode === 'pwm' ? `${Math.round(brightness * 100)}%` : (on ? 'ON' : 'OFF')}
      </div>
    </div>
  );
}
